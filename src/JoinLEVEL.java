...
public class JoinLEVEL {
    public static ArrayList<FlightLEVEL> joinLEVEL(ArrayList<FlightARMS> flights, ArrayList<String> matriculas) {
        ArrayList<FlightLEVEL> flightListLEVEL = new ArrayList<>();
        if (flights.isEmpty()){
            return null;
        }else {
            for (String matricula : matriculas) {
                for (FlightARMS flight : flights) {
                    if (matricula.equals(flight.matricula)) {
                        if (flight.dest.equals("BCN")) { //Vuelo de llegada
                            Boolean find = false;
                            for (FlightARMS flight1 : flights) {
                                int pernocta = 0;
